...
const CONFIG = {
  ...
  aircall: {
    ...
    answeringServiceNumber: "+12065904488",  // external call answer service
  },
  agents: ["Jeff Taylor", "Nathan Whaley", "Divine Monkam"],
  ...
  excludePostCallAgents: ["Jarred"],
  // Aircall lines to exclude from SMS activity tracking
  excludeSMSLines: ["nonpro sales (post close)", "Jarred Bullard", "Ben Gratza", "Danny Brady", "Beth Ann Hunt", "Soren Dahl", "Brittany Bowles", "Curtis Smith", "Ian Storz", "Brian Broyles", "Pierce Jens", "Amanda D"],
...
function fetchZendeskStatus() {
  ...
  // Find who the "Other" solvers are so we can label the row (e.g. "Other (Katie)")
...
function setupAgentMapTab(ss) {
  ...
  const agents = [
    ["Jeff Taylor", "Jeff Taylor", "Jeff Taylor", "Anchor"],
    ["Nathan Whaley", "Nathan Whaley", "Nathan Whaley", "Full-Time (Mar 1)"],
    ["Divine Monkam", "Divine Monkam", "Divine Monkam", "Ramping"],
    ["Katie M", "Katie M", "Katie M", "Manager"],
  ];
...
const METRICS_LOG_HEADERS = [
  ...
  // e.g. "Solved: Jeff", "Solved: Nathan", "Solved: Divine"
...
//    {"Jeff Taylor":"SPREADSHEET_ID","Nathan Whaley":"SPREADSHEET_ID","Divine Monkam":"SPREADSHEET_ID"}
// ═══════════════════════════════════════════════════════════
...
function updateAgentDashboards(zendesk, aircall, csat, postCall) {
  ...
  agents.forEach(agent => {
    ...
    // Clean up old sheet names from previous versions (e.g. "Jeff - Agent Command Center")
